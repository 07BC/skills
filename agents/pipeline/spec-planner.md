...
- [ ] Every acceptance criterion in the spec is covered by at least one task —
      either by an implementation task (R-mapped) or a paired test task

### Traceability spine (when the spec frontmatter has `covers:`)

A spec produced from a GitHub child sub-issue carries `covers: [<frozen AC IDs>]`
and each task carries `implements: [<ids>]`. When present, verify:

- [ ] Every ID in the spec's `covers:` appears in at least one task's
      `implements:` list (no unplanned AC)
- [ ] No task's `implements:` references an ID outside `covers:` (no scope creep)
- [ ] Every task that implements an AC is paired with a test task whose `@Test`
      will carry the matching `// AC: <id>` annotation

Any failure here is `PLAN NEEDS AMENDMENT` — the deterministic
`check-traceability.sh` gate will reject it downstream, so catch it now.
