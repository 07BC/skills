...
> Primary unit-test target: **ViewModels** — they contain all business logic and have no SwiftUI dependency.
...
extension Tag {
    ...
    @Tag static var repositories: Self
...
AppNameTests/
  ...
  Tests/
    ...
    Repositories/                   ← One <Feature>RepositoryTests.swift per repository
    ViewModels/                     ← One <Feature>ViewModelTests.swift per screen (primary)
...
- **Never use `Task.sleep` to wait for state to settle.** Flaky by construction. Use `confirmation { }` for callback/notification APIs, `withCheckedContinuation` for completion handlers, or `await` the ViewModel/repository method directly.
...
|---|---|---|
| Shared mutable system state (UserDefaults, Keychain, FileManager, NotificationCenter, in-memory caches) | `actor` | Read/modify/write needs serialisation across calls. Locks are not approved and force synchronous call sites that clash with `async` test bodies. |
| Stateless repository with call recording | `actor` | Mutable `fetchCallCount` + result stubs need actor isolation across concurrent calls. |
...
| `@Observable` SwiftUI ViewModel (`@MainActor`-isolated) | The real type, exercised via `await` | Don't mock your own `@Observable` ViewModels. Construct them with mocked repositories and `await` their methods. |

### Worked example: `actor MockFeatureRepository`
...
| `MockFeatureRepository` | `FeatureRepositoryProtocol` |
...
Each mock with mutable state is an `actor`. Call counts and result stubs are actor-isolated and `await`-ed in tests.
...
A `@MainActor @Observable final class` ViewModel is the correct design for a SwiftUI screen ViewModel. Testing it does **not** require putting `@MainActor` on the test function.

**Pattern 1 — Construct in-body, `await` the methods**

The ViewModel is not stored as a suite property — `@MainActor` types are not safely stored on a nonisolated `Sendable` struct. Construct per-test in the body.

```swift
@Suite(.tags(.viewModels))
struct FeatureViewModelTests {

    @Test("Loads items on refresh")
    func loadsItemsOnRefresh() async throws {
        let repo = MockFeatureRepository()           // actor
        let sut = await FeatureViewModel(repository: repo)  // @MainActor init
...
When the ViewModel exposes synchronous `@MainActor`-isolated properties, reading them from a nonisolated test requires a hop. Wrap only the property read — not the whole test body.

```swift
@Test("Surfaces error state when repository fails")
func surfacesError() async throws {
    let repo = MockFeatureRepository()
    await repo.stub(.failure(.network(URLError(.notConnectedToInternet))))
...
struct FeatureViewModelTests {
    ...
    init() async throws {
        ...
        await repo.stub(.success(.fixture()))
    }

    @Test("Initial state has empty items")
    func initialStateIsEmpty() async throws {
        let repo = MockFeatureRepository()  // fresh repo — no pre-stubbed data
        let sut = await FeatureViewModel(repository: repo)
        let items = await MainActor.run { sut.items }
...
Do not store `@MainActor`-isolated ViewModels as suite properties — construct them per-test in the body instead.
...
struct FeatureViewModelTests {
    ...
    @Test("Refresh populates items on success")
    func refreshPopulatesItems() async throws {
        let repo = MockFeatureRepository()
    ...
    func refreshSetsErrorOnFailure() async throws {
        let repo = MockFeatureRepository()
        await repo.stub(.failure(.network(URLError(.notConnectedToInternet))))
        let sut = await FeatureViewModel(repository: repo)
...
Repository tests exercise data-layer logic: DTO mapping, HTTP error translation, query parameter encoding. They are off-main and need no `MainActor.run`.
...
struct FeatureRepositoryTests {
    ...
    @Test("fetch passes correct page parameter to API client")
...
No `@MainActor` needed — pure async repository tests run off-main.
...
```swift
@Test("Posts didUpdate notification when items load")
...
func refreshesToken() async throws {
    let sut = AuthRepository(api: MockAuthAPIClient())
...
```swift
@Suite("MockFeatureRepository concurrency", .tags(.repositories))
struct MockFeatureRepositoryConcurrencyTests {

    @Test("Concurrent fetch calls succeed without data race")
    func concurrentCallsSucceed() async throws {
        ...
        await withTaskGroup(of: Void.self) { group in
            for _ in 0..<50 {
                group.addTask {
                    _ = try? await repo.fetch(page: 1)
                }
            }
        }

        let callCount = await repo.fetchCallCount
        #expect(callCount == 50)
    }
}
```
...
@Test("ViewModel conforms to protocol")
func conformsToProtocol() async throws {
    #expect(sut is FeatureViewModelProtocol)
}
```
...
// GOOD — verify the value reached the repository with the correct key
@Test("Persists value to repository with correct key")
...
```swift
@Suite(.tags(.repositories))
struct ItemStorageRepositoryTests {
    ...
    func savesAndFetchesItem() async throws {
        ...
        let sut = ItemStorageRepository(modelContainer: container)
