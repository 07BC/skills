...
`ettrace` is interactive and needs a real TTY to answer its prompts. Claude Code's Bash tool is non-interactive, so run the `ettrace` capture step yourself in a terminal (or via a wrapping script that pre-answers prompts); a run without a TTY can exit without producing a usable trace.
