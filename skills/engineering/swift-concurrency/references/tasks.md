...
### When to store a Task reference

An unassigned `Task { }` is fire-and-forget — you lose the ability to cancel, await, or know when it finishes. If you cannot articulate why it is safe to lose track of a Task, store the reference.

| Scenario | Store the Task? |
|---|---|
| Model/business logic kicks off async work | **Yes** — always |
| Work can be triggered multiple times (pull-to-refresh, pagination) | **Yes** — cancel-then-reassign |
| One-shot fire-and-forget (analytics event, logging) | No — acceptable to discard |
| Long-running background work (polling, WebSocket) | **Yes** — must be cancellable |

### Cancel-then-reassign pattern

Prevents duplicate in-flight work and ensures cleanup:
...
### Prefer `async` functions over Task-spawning functions

The `func foo() { Task { } }` pattern is a concurrency escape hatch. Models and business logic should expose `async` functions directly. The only places `Task { }` should appear are at boundaries — SwiftUI `.task`, UIKit lifecycle, or delegate callbacks.

```swift
// ❌ Anti-pattern: sync wrapper hides async work
func loadProfile() {
    Task { profile = await service.fetch() }
}

// ✅ Preferred: async function, testable and cancellable
...
### The .task modifier (preferred)

Automatically manages task lifetime with view lifecycle. Cancels when the view disappears.
...
```swift
// ❌ Avoid: manual Task in onAppear — no automatic cancellation
.onAppear {
    Task { await model.load() }
}

// ✅ Prefer: automatic lifecycle management
.task { await model.load() }
```
...
### Injecting a Clock for testable time-dependent code

When production code involves delays (polling, debounce, retry), inject a `Clock` so tests can control time:
...
Every `Task.sleep` in a test is a sign that the production code's async boundary isn't surfaced in a testable way. Use these patterns instead.

### Pattern 1: Direct `await` (preferred)

When the model exposes `async` functions, tests simply await them:

```swift
@Test func itemsLoad() async {
...
When production code uses `Task { }` internally and you cannot change it, use Swift Testing's `confirmation()`:

```swift
@Test func profileFetchesOnLoad() async {
...
See the [Injecting a Clock](#injecting-a-clock-for-testable-time-dependent-code) section above. Use a test clock to advance time instantly instead of waiting real seconds.

### Pattern 4: Test cancellation behaviour

Verify that stored Tasks actually respect cancellation:

```swift
@Test func cancelStopsLoading() async throws {
...
- [ ] Model functions are `async` — not wrapping work in `Task { }`
- [ ] SwiftUI views use `.task` / `.task(id:)` — not `onAppear` + `Task { }`
- [ ] Any stored `Task` is cancelled before reassignment
- [ ] Time-dependent code accepts an injected `Clock`
- [ ] Tests use `await` directly — no `Task.sleep(.seconds(N))`
- [ ] Tests use `confirmation()` only when `Task { }` is unavoidable
...
1. **Prefer `async` functions** over `func foo() { Task { } }` wrappers
2. **Store Task references** when cancellation or lifecycle management matters
3. **Cancel-then-reassign** when a Task can be triggered multiple times
4. **Use `.task` / `.task(id:)`** in SwiftUI — not `onAppear` + `Task { }`
5. **Check cancellation regularly** in long-running tasks
6. **Use structured concurrency** (avoid detached tasks)
7. **Inject a `Clock`** for time-dependent code to enable deterministic testing
8. **Choose the right tool**: `async let` for fixed, TaskGroup for dynamic
9. **Handle errors explicitly** in throwing task groups
10. **Set priority only when needed** (inherit by default)
