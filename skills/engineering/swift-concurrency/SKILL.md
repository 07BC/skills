...
```swift
// Caller is @MainActor — the Task closure inherits MainActor isolation.
// fetchData() is a @concurrent async func, so it hops to the global executor
// for the network call and returns the result back to the main actor.
Task {
...
- After refactors:
  ...
  - **If Xcode is open**, call `mcp__xcode__XcodeListNavigatorIssues` to surface any live concurrency warnings the compiler surfaced in the editor — these often include isolation violations and Sendable diagnostics that only appear after the file is re-checked. Load the schema first: `ToolSearch("select:mcp__xcode__XcodeListNavigatorIssues,mcp__xcode__XcodeRefreshCodeIssuesInFile")`.
