...
Entries file is newline-separated; pass "-" to read from stdin. Blank lines
are dropped. Entries already present verbatim in the target file (anywhere)
are skipped — naive but safe.

Surviving entries are appended under a `## YYYY-MM-DD` heading. If the
heading already exists, entries are appended within that section; otherwise
a new section is added at the end of the file. The parent directory of
--target is created if missing.

The category→filename mapping is intentionally NOT baked in here, because
obsidian-learn and session-saver use different maps. Callers compute their
target path before invoking.

CANONICAL COPY. A duplicate lives at:
    skills/obsidian/session-saver/scripts/kb_append.py
Keep both in sync.
"""
