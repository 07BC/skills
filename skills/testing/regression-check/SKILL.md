...
description: Audits in-progress code changes or written plans for side effects and regressions BEFORE they are committed or implemented. Catches broken callers (blast radius), behavioural ripples through shared state, observers, KVO, NotificationCenter, scenePhase, and concurrency or threading regressions. Always use this skill when the user says "audit for regressions", "check side effects", "what could this break", "is this safe to commit", "what am I missing", or asks Claude to verify that a plan or pending change won't break anything else. Also trigger proactively right after a plan document is written and before any code is generated for it, and right before a commit when significant changes are staged. Routing scope — fires for standalone regression / side-effect analysis work (one-off edits, single-file reviews, quick fixes, ad-hoc questions). For full-feature work driven from a Jira ticket or a multi-task spec, defer to spec-pipeline which runs the engineer / test-writer / concurrency-auditor / task-reviewer sub-agents in a worktree.
---
